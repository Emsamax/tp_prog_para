 ...
 */
 ...
 int main(int argc, char *argv[]) {
   ...
   MPI_Type_create_resized(type_temp, 0, taille_deplacement, &type_tranche);
   MPI_Type_commit(&type_tranche);  
 
   /* Le processus 0 distribue dans AL les tranches horizontales de A */
   MPI_Scatter(A, N*NL, MPI_DOUBLE, AL, N*NL, MPI_DOUBLE, 0, MPI_COMM_WORLD);
 
   /* Le processus 0 distribue dans BL les tranches verticales de B */
   MPI_Scatter(B, 1, type_tranche, BL, NL*NL, type_tranche, 0, MPI_COMM_WORLD);
   ...
   for (iter=0; iter<Nprocs; iter++) {
     ...
     if (rang != iter) {
      MPI_Sendrecv(AL, NL, MPI_DOUBLE, iter, etiquette, TEMP, NL*N, MPI_DOUBLE, iter, etiquette, MPI_COMM_WORLD, &statut);
    ...
    */
    MPI_Gather(CL, N*NL, MPI_DOUBLE, CL, 1, type_tranche, 0, MPI_COMM_WORLD);
